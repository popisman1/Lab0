...
public class LinkedList<T> implements PrintableList<T> {
	...
	private class ListNode {
		...
		/**
		 * Recursively builds a forward string representation: "e1, e2, e3"
		 */
		...
		/**
		 * Recursively builds a reverse string representation: "en, ..., e2, e1"
		 */
	...
	public T removeFirst() {
		if (isEmpty()) {
			throw new ListEmptyException("Unable to remove first element, list is empty.");
		}
		T value = head.data;
		head = head.next;
		if (head == null) {
	...
	public T removeLast() {
		if (isEmpty()) {
			throw new ListEmptyException("Unable to remove last element, list is empty.");
		}
